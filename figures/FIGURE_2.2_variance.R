...
neuron_vars <- read_tsv('data_/trajectories/ALL_lineage_pt_r2.tsv')
ac_vars <- read_tsv('data_/trajectories/ALL_astrocyte_telen_diff_r2.tsv')
...
rna_neuro <- read_rds('data_/trajectories/RNA_neuro_dpt_srt.rds')
rna_genes <- rna_neuro %>% FindVariableFeatures(nfeatures=10000) %>% VariableFeatures()
       
regvars_df <- neuron_vars %>%
...
p1 <- ggplot(regvars_df, aes(mods, count)) +
...
p2 <- ggplot(nvars_df, aes(mods, count)) +
    geom_bar(size=0.1, fill='darkgrey', color='black', stat='identity') +
    article_text() +
    scale_axis_rangeframe() + theme_rangeframe() +
...
out_df <- bind_rows('region'=regvars_df, 'differentiation'=nvars_df, .id='axis')
out_df %>% write_rds('data_/tables/ed15e_variance_specificity.tsv')  
